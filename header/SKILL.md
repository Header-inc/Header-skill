...
Local, read-only — nothing leaves the machine. Run **all eight** scans. `<AUDIT>` is `header-audit`, in the same `bin/` dir as the preamble's `HEADER_BIN`.
