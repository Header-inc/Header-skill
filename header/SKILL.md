...
Why model **and** effort: the System Card's headline is that Opus 4.8 at a *lower* effort can match the prior model at a *higher* one — so the win is often "same quality, cheaper," which only a model+effort A/B can surface. **2-arm by default; `--sweep` offers a 3rd arm** at the next effort up (interactive y/N on a TTY) — the analyzer is pairwise, so `analyze`/`report --vs C` compares the control against that arm (a side `result-vs-C.json`; the canonical `result.json` stays the A-vs-B). Detection reads `ANTHROPIC_MODEL` → settings `model` → the **most recent model in the user's transcripts** (what actually ran); only if all are empty does it assume `claude-opus-4-7` and **say so** (pass `--from` to correct — arm A is the control). When `merge` finds a B-wins engine swap it **offers to write** `model` + `effortLevel` into `.claude/settings.json` (shows a diff, asks first; `max` stays advisory). The proof needs a git repo (it mines real fixes); outside one, point the user at the card.
