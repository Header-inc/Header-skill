...
- `CONTEXT-TAX registry <count> <bytes> <est_tokens>` — the analogous tax for slash-commands and subagents: their **bodies** are on-demand (`ONDEMAND`), but their frontmatter (name + description) is injected into the command/agent registry each session. This is the **only** always-loaded cost of those files. Emitted by `harness` (static — shows on a fresh clone, transcript-independent); put it on the scorecard next to the skills tax. Never graded, and never folded into the `FILE` per-turn sum.
