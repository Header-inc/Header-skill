...
1. **Runs the audit** locally — `header-audit harness` (prompt-config debt, `CLAUDE.md`/`AGENTS.md` size, model declaration, cargo-cult patterns, Bash-tool permission posture, the always-loaded context tax — skills + command/subagent registry frontmatter, split repo→📦 project / user→💻 local), `header-audit deps` (ecosystems, install-cooldown gate, supply-chain posture), `header-audit waste` (usage accounting from your own session transcripts — MCP servers and skills you pay for but never use, tool error rates, compaction pressure), `header-audit rails` (determinism guardrails — pre-commit gate, test ratchet, compounding memory, round-trip invariant — plus **inert machinery**: a gate that is present and *cannot fail*, e.g. checks ending in `|| true`, CI with `continue-on-error`), `header-audit drift` (**invariant coverage** — the one scan that asks whether your machinery covers what your *architecture* depends on, rather than whether machinery exists: it finds a value pipeline in your source (wire schema → column → import mapping → export query → serializer) and reports whether anything asserts that a field survives it intact, plus schemas that silently drop unknown keys — the reason such drift stays invisible until a human notices wrong numbers), `header-audit silence` (the **silence axis** — config vars the code reads but nothing declares; exception handlers with empty bodies: places the system discards information without telling anyone), `header-audit retro` (the **coach** scan — behavioral mining of your own sessions: edit-thrash, gotcha volume, git-workflow tells, ranked capability nudges, and **co-change drift** mined from git history: the files that always move together, and the commits where one didn't), and `header-audit grade` (deterministic composite **setup grades** — an `A+`…`F` mark over the five scorecard axes for **📦 project** (the repo's checked-in config — the reproducible headline) and a separate **💻 local harness** mark for your machine (`~/.claude`, the model you run, tool versions); the split keeps explicit what's being graded).
...
3. **Cross-references** the briefing `summary` (its key developments) against your actual stack (read from package manifests, `CLAUDE.md`, README, recent git activity).
4. **Leads with the coach, demotes the audit.** The output opens with a behavioral read of your own sessions — what this week's work was, ⚠️ gotchas & pitfalls, and 🎯 best practices **ranked by what your sessions show** (frustrated by branch-juggling → worktrees; tests breaking → a guardrail; repeated pitfalls → `/header wrapup` memory). The config scorecard — headlined by the **📦 project setup grade** (`A+`…`F`, the reproducible repo grade shown on howsmyaicoding.com), with a separate **💻 local harness** grade for your machine beside it — and recommendations follow as a 🧹 hygiene tail. It renders in one clear register for everyone — plain language, outcomes first, depth available but never crowding the lead. Findings split into **apply-now** (deletions, the supply-chain gate, security patches — deterministic, low-risk) and **`[Experiment]`** _(beta)_ — changes whose payoff must be proven (model swaps, major upgrades, behavioral rewrites). For these, `header-experiment` (see below) runs a local A/B against your tasks; the skill itself doesn't auto-experiment yet. It also surfaces opinionated **determinism rails** — guardrails Header endorses for AI-written code (a pre-commit gate, a test ratchet, compounding memory, a round-trip invariant over any value pipeline it finds) — and offers to install them; the pitch is **prose asks, a guardrail enforces** (a `CLAUDE.md` rule the agent might forget vs. a hook that can't be skipped).
5. **Offers to implement** the apply-now items right there.

**Your code never leaves the machine** — the audit and the cross-reference are always 100% local (no code, no file contents, no diffs). The only thing fetched is the briefing from `joinheader.com`. Enrichment is your choice: pick **generic** and nothing project-derived egresses at all; pick **custom** and the skill sends a one-line stack summary (e.g. *"Python/FastAPI + React"*) — nothing else — so the briefing can be tuned to your stack.

On the first run in a new repo, interactive sessions ask once: **generic agentic-coding recommendations, or ones tuned to this codebase?** Choose custom and the skill sets up a free account with no signup screen (an anonymous trial — or paste your key if you're already a Header user), builds a topic for your stack, and from then on every audit in that repo is enriched by *your* briefing. Convert the anonymous account to a full one anytime via a claim link; your API key and topics are preserved.
